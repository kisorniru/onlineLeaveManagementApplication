				...
				function reload(form)
					{
						...
						self.location='ListOfUserBlood.php?cat=' + val ;
					}
			...
			<div id="wrapper">
				...
				<div id="sidebar-wrapper">
					<ul class="sidebar-nav">
						...
						<li>
							<a style="color:#DAA520;" href="ListOfUserBlood.php">Blood Group</a>
						</li>
						<li>
					...
					<div class="container-fluid">
						<div class="row">
							<div class="col-lg-12">
								...
								<h1 align="center">Employee Blood Group Information</h1>
								<?php
									...
									$bldResult = $objEmployee->getEmpBloodInfo();
									...
									{
										$objEmployee = new Employee();
										$singleResult = $objEmployee->getSingleEmpBloodInfo($cat);
									}
									else
									{
										$objEmployee = new Employee();
										$singleResult = $objEmployee->getAllEmpBloodInfo();
									}
								...
								<form role="form" action="" method="" >
									...
									<div class="col-lg-6 col-md-offset-6 form-group">
										...
										<div class="input-group">
											...
											echo "<select class='form-control' name='cat' id='empDptName' onchange=\"reload(this.form)\"><option value=''>Select All</option>";	
													...
													while ($row = mysqli_fetch_array($bldResult))
													{
														if($row['bId']==@$cat)
														{
															?>
																<option selected value='<?php echo $row['bId']; ?>'><?php echo $row['bName']; ?></option>
															<?php
														}
														else
														{
									...
									<div class="form-group">
										...
										<?php
										echo "<table class='table table-bordered table-hover table-striped' name='subcat'><thead><tr class='success'><th>Name</th><th>Department Name</th><th>Designation Name</th><th>Blood Group</th><th>Phone Number</th></tr></thead><tbody>";
										
												while ($row2 = mysqli_fetch_array($singleResult))
												{ 
													?>
														<tr>
															<td><?php echo $row2['eFirstName']." ".$row2['eLastName'] ?></td>
															<td><?php echo $row2['dptName'] ?></td>
															<td><?php echo $row2['desiDesignationName'] ?></td>
															<td><?php echo $row2['bName'] ?></td>
															<td><?php echo "P: ".$row2['ePhoneNumberPersonal']."<br/>O: ".$row2['ePhoneNumberOffice'] ?></td>
														</tr>
													<?php
												}
											echo "</tbody></table>";
